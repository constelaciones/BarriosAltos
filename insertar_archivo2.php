...
include("conecta.php");
        ...
        echo $binario_tipo."tipoDeArchivo"."</br>";
        echo $binario_peso."pesoDeArchivo"."</br>";
        ...
        //if(!empty($_POST['nombre_archivo'])){
            ...
            $CorrectaGeoposicion = false;
            $CorrectoPeso = false;
            $CorrectoTitulo = false;
    ...
    if( $nombre == NULL ){
        ...
        if(!empty($tagnuevo[$i])){
            ...
            if($tagsSeleccionados > 1){
                ...
                $consulta_insertarConTags = "INSERT INTO documento (titulo_registro, peso, extension, descripcion, nombre_archivo, longitud, latitud, URL, selectedradio, tag, usuari) VALUES 
                ('$nombre', '$binario_peso', '$binario_tipo','$descripcion','$binario_titulo', '$longitud','$latitud','$URL', '$selectedradio', '$cadenaTags', '$usuari')";
                mysql_query($consulta_insertarConTags,$conexion) or die("que no que no tags.");
                

                }else{
                    $consulta_insertar = "INSERT INTO documento (titulo_registro, peso, extension, descripcion, nombre_archivo, longitud, latitud, URL, selectedradio, tag, usuari) VALUES 
                    ('$nombre', '$binario_peso', '$binario_tipo','$descripcion','$binario_titulo', '$longitud','$latitud','$URL', '$selectedradio', '$tag[0]', '$usuari')";
                mysql_query($consulta_insertar,$conexion) or die("que no que no.");
                }
                
                echo "Tu documento se ha subido a la red correctamente, ya puedes visionarlo!";
                }
